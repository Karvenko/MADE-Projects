...
class Treap():
    ...
    def move_front(self, left, right):
        tmp, end = self.split(right)
        start, mid = self.__split(tmp, left - 1)
        tmp = self.merge(mid, start)
        self.root = self.merge(tmp, end) 
