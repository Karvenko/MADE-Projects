...
class Graph:
    ...
    def make_fb_step(self, distances):
        # print(distances)
        new_dist = distances.copy()
        for idx, node in enumerate(self.nodes):
            for edge in node.edges:
                # print(idx, edge[0], edge[1], distances)
                new_dist[edge[0]] = min(new_dist[edge[0]],
                                        INF if distances[idx] == INF else distances[idx] + edge[1])
                # print(new_dist)
        return new_dist

    def ford_bellman(self, start):
        distances = [INF] * self.n_nodes
        # final_dist = [INF] * self.n_nodes
        distances[start] = 0
        for _ in range(self.n_nodes - 1):
            distances = self.make_fb_step(distances)

        check_dist = distances.copy()
        for _ in range(self.n_nodes - 1):
            check_dist = self.make_fb_step(check_dist)
            for idx in range(len(check_dist)):
                if check_dist[idx] < distances[idx]:
                    check_dist[idx] = NO_SHORTEST_WAY
