...
class LinkedMap():
    ...
    def add(self, key, value):
        ...
        if res == None:
            new_node = DLNode(key, value, self.last)
            if self.last != None:
                self.last.next = new_node
            self.last = new_node
            _ = self.hashmap.add(key, new_node)
        else:
            res.value = value
            _ = self.hashmap.add(key, res)
    ...
    def delete(self, key):
        ...
        if node.key == self.last.key: #last element
            self.last = node.prev
            if self.last != None:
                self.last.next = None
            _ = self.hashmap.delete(key)
            return
        if node.prev == None: #first element
            node.next.prev = None
            _ = self.hashmap.delete(key)
            return
...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        if line[:2] == 'pu': #put
            ...
            linkedmap.add(key=key, value=value)
        elif line[0] == 'd': #delete
            linkedmap.delete(line.split()[1])
        elif line[0] == 'g': #get
            res = linkedmap.get(line.split()[1])
            if res != None:
                results.append(res)
            else:
                results.append('none')
        elif line[:2] == 'pr': #prev
            res = linkedmap.get_prev(line.split()[1])
            if res != None:
                results.append(res)
            else:
                results.append('none')
        elif line[0] == 'n':
            res = linkedmap.get_next(line.split()[1])
            if res != None:
                results.append(res)
            else:
                results.append('none')
